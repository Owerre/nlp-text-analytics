...
def webapp_data(data, threshold=None):
    ...
    for card in cards:
        ...
        sent = (
            ...
            + df_sent.loc[2]
            + 'varsep'
            + df_sent.loc[len(df_sent) - 1]
            + 'varsep'
            + df_sent.loc[len(df_sent) - 2]
            + 'varsep'
            + df_sent.loc[len(df_sent) - 3]
        )
        ...
        total_dict = dict()
        for pos, neg, neu in zip(
            pos_dict.items(), neg_dict.items(), neu_dict.items()
        ):
